...
(() => {
  ...
  const zh = {
    ...
    "Build a searchable sky index.": "建成可查询的天空索引。",
    "Warehouse is a scanning tool for astronomical data. Point it at a local directory, an S3-compatible bucket, or OSS storage. It inventories every file it finds, extracts each file's sky coverage from supported metadata, and writes the current results into Elasticsearch ast_* indices, retaining evidence for every scan. It never reads or copies scientific arrays.": "Warehouse 是ASA项目开发的一个天文数据扫描工具。可以把本地目录、S3 兼容存储或 OSS 交给它：它会清点发现的每一个文件，只从支持的元数据中提取每个文件的天空覆盖，把当前结果写入 Elasticsearch 的 ast_* 索引，并为每次扫描保留证据。它从不读取或复制科学数据数组。",
    "Once a scan lands in the index, the sky-position questions come from your application: which files cover this region, where do they live, and why did they match. Assets asks this in production; your code can query the same indices.": "扫描结果写入索引后，按天区反查等问题由你的应用来回答：有哪些文件覆盖该区域、它们存放在哪里、为什么被命中。生产环境由 Assets 直接这样查询；你的代码也可直接读取同一套索引。",
    ...
    "Choose how to read your sky coverage": "你的文件里，天空位置记在哪里？",
    ...
    "Where to find layers, files, and coverage": "直接查索引：从图层找到文件",
