...
(() => {
  ...
  const zh = {
    ...
    "Choose the mode that matches the spatial information in your files. Each ScanPlan covers one source and one CoverageLayer, using one extraction mode. Warehouse checks the plan before accessing the source.": "根据文件记录位置的方式，选择下面一种提取模式。每份 ScanPlan 指定一个数据源、一个 CoverageLayer 和一种模式；Warehouse 会先检查计划，再访问数据源。",
